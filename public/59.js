    ...
    methods: {
        prepareForm: function prepareForm(res) {
            ...
            var _ref3 = [res.orders.filter(function (order) {
                ...
                var isMyBranch = function isMyBranch() {
                    if (_this2.$store.getters.auth('DVALead')) return true;
        ...
        initializeReminders: function initializeReminders() {
            ...
            this.orders.forEach(function (order) {
                ...
                _this3.promiseCalls.push({
                    order_id: order.id,
                    user_id: _this3.dva_id,
                    customer_id: order.customer.id,
                    date: null
                });
            });
        ...
        logReminder: function logReminder(index) {
            ...
            (0, _api.post)('/api/reminder', { reminders: [reminder] }).then(function (_ref6) {
                var data = _ref6.data;
                return data.saved ? _this5.logPromiseCall(_this5.promiseCalls[index]) : _this5.displayErrorMessage('Error Logging reminders!');
            });
        },
        logPromiseCall: function logPromiseCall(promiseCall) {
            ...
            if (!!promiseCall.date) {
                ...
                (0, _api.post)('/api/promise_call', promiseCall).then(function (_ref7) {
                    var data = _ref7.data;
                    return data.saved ? _this6.done("Reminder Logged!, Promise call added!") : _this6.displayErrorMessage('Error Logging promise call!');
                });
                                ...
                                function(payment) {
                                  return _c("td", [
                                    _vm._v(
                                      "\n                                    " +
                                        _vm._s(
                                          _vm.$formatCurrencyCurrency(payment)
                                        ) +
                                        "\n                                "
                                    )
                                  ])
