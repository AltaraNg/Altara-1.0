<!--<template>
    <transition name="fade">
        <div id="reminder">
            ...
            <transition name="fade">
                <div v-if="customer && show" class="z-1 position-relative">
    ...
    export default {
        ...
        methods: {
            async updateView(data) {
                ...
                if (!!customer.length) {
                    customer = customer[0];
                    if (!customer.document) customer.document = {id_card_url: "", passport_url: ""};
            ...
            processForm(customerId) {
                this.show = false;
                this.$LIPS(true);
                get(`/api/customer/lookup/${customerId}`)
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            sendPaymentCompleteSMS() {
                return; //TODO: i was asked to disable the sms sent when a customer has completed payments.
                let messageBody = `Dear ${this.activeOrder.customerName}, you have successfully completed ` +
                    `your payment for ${this.activeOrder.order.store_product.product_name}. ` +
                    `Thanks for patronizing us.`,
                    message = new Message(messageBody, this.activeOrder.customer.telephone);
